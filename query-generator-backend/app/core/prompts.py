...
def build_system_prompt(
    ...
    prompt_parts = [
        ...
        "2. NEVER invent or assume tables/columns based on common naming conventions or",
        "   prior knowledge. If the schema says the table is `vw_users_masked` with a",
        "   `created_at` column, do NOT write `users.signup_date` — use what is given.",
        "3. If the context does NOT contain a table or column needed to answer the",
        "   question, do NOT guess. Return `\"sql\": null` and use the `explanation`",
        "   field to list exactly which tables/columns are missing.",
        "",
