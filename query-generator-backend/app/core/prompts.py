...
def build_system_prompt(
    ...
    prompt_parts = [
        ...
        "   `=== RELEVANT CONTEXT ===` section below (under `--- DATABASE SCHEMA ---`).",
        "2. NEVER invent or rename tables/columns. If the schema says the table is",
        "   `vw_users_masked` with a `created_at` column, write `vw_users_masked.created_at` —",
        "   do NOT write `users.signup_date`, do NOT invent a `signup_date` column.",
        "",
        "SEMANTIC INFERENCE — allowed (and expected):",
        "Column comments in this catalog are often just `-- <column_name> column`",
        "(uninformative). You SHOULD apply common-sense conventions to interpret real",
        "columns that are present in the context:",
        "   • `created_at` on a users/accounts table → user sign-up / registration time.",
        "   • `created_at` on an orders/trades/transactions table → row creation time.",
        "   • `processed_at` on an orders table → order fill / completion time.",
        "   • `updated_at`, `deleted_at` → last update / soft-delete time.",
        "   • `user_id`, `order_id`, etc. → foreign keys to the obvious entity.",
        "   • `side` in ('buy','sell'), `type` in ('otc','spot'/etc.) on orders → market side / order kind.",
        "Use these conventions confidently when the column actually exists in context.",
        "",
        "3. Return `\"sql\": null` ONLY when a literal table or column you need is",
        "   ABSENT from the context — not when you are merely unsure about the meaning",
        "   of a column that is clearly named. When you do return null, list the",
        "   specific tables/columns that are missing.",
        "",
